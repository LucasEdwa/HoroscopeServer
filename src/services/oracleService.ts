...
export async function saveOracleQuestion(data: Omit<OracleQuestion, 'id' | 'created_at'>): Promise<OracleQuestion> {
  try {
    const query = `
...
export async function getOracleQuestionsByEmail(email: string): Promise<OracleQuestion[]> {
  try {
    const query = `
...
export async function getOracleQuestion(params: { email?: string; id?: number }): Promise<OracleQuestion | OracleQuestion[] | null> {
  try {
    if (params.id) {
      ...
      return getOracleQuestionsByEmail(params.email);
    } else {
...
export async function deleteOracleQuestion(id: number): Promise<OracleQuestion> {
  try {
    const existingQuestion = await getOracleQuestion({ id }) as OracleQuestion | null;
    if (!existingQuestion) {
...
export async function getAllOracleQuestions(limit?: number, offset?: number): Promise<OracleQuestion[]> {
  try {
...
export async function askOracleQuestion(input: { email: string; question: string; chart?: any }): Promise<{ question: string; answer: string }> {
  try {
